...
<body class="sigup-one" onload="Captcha();">
    ...
    <div id="load_screen"> 
        ...
        <p class="xato-loader-heading">기업회원 회원가입</p>
    </div>
    <!--  Loader Ends -->
    <!-- Main Body Starts -->
            ...
            <div class="row main-login-one h-100">
                <div class="col-xl-5 col-lg-6 col-md-6 col-sm-12 p-0">
                    <div class="login-one-start">
                        <h6 class="mt-2 text-primary text-center font-30">기업회원 회원가입</h6>
                        <p class="text-center text-muted mt-3 mb-3 font-14">회원정보입력후 계정을 등록하세요</p>
                        <h6 class="mt-2 text-primary font-20">사업자 등록번호 조회</h6>
                        ( -를제외한 사업자등록번호를 작성해주세요  )
                        <div class="login-one-inputs mt-1">
                            <input type="text" id="companyNum" placeholder="사업자등록번호를 입력하세요"/>
                            <i class="las la-lock"></i>
                        </div>
                        <div id="companyStatus"></div>
                        <button type="button" id="companyCheckBtn" class="btn btn-sm mt-2 btn-primary">진위여부확인</button>
                        
                        
                        <form action="/member/companyjoin" id="form" method="post">  
                        
                        <h6 class="mt-2 text-primary font-20">아이디</h6>
                        <div class="login-one-inputs mt-1">
                            <input type="text" name="memId" id="memId" placeholder="id를 입력하세요"/>
                            <div id="memCheck"></div>
                            <i class="las la-envelope"></i>
                        </div>
                        <h6 class="mt-2 text-primary font-20">비밀번호</h6>
                        <div class="login-one-inputs mt-1">
                            <input type="password" id="memPw" name="memPw" placeholder="비밀번호를 입력하세요"/> 
                            <i class="las la-lock"></i>
                        </div>
                        ...
                        <div class="login-one-inputs mt-1">
                            <input type="text" name="memNick" id="memNick" placeholder="닉네임을 입력하세요"/> 
                            <i class="las la-lock"></i>
                        </div>
                        <h6 class="mt-2 text-primary font-20">휴대전화번호</h6>
                        <div class="login-one-inputs mt-1">
                        ...
                        <div class="login-one-inputs mt-1">
                            <input type="text" id="memEmail" name="memEmail" placeholder="이메일을 입력하세요"/> 
                            <i class="las la-lock"></i>
                        </div>
                        <button type="button" id="mailBtn" class="btn btn-sm mt-2 btn-primary">이메일인증</button>
                        <br>
                        <div class="login-one-inputs mt-1">
                            <input type="text" id="joinNum" name="joinNum"
							placeholder="인증번호" style="width: 35%">
							<i class="las la-lock"></i>
                        </div>
                        <div id="status"></div>
						<div id="timer"></div>
                           ...
                           <button type="button" id="numCheckBtn" class="btn btn-sm mt-2 btn-primary">인증번호확인</button>
                        ...
                        <div class="login-one-inputs mt-2">
                            <fieldset>
                                <legend>성별</legend>
                                <div>
                                  <input type="radio" id="male" name="memGender" value="mail"
                                         checked>
                                  <label for="huey">남자</label>
                                  <input type="radio" id="femail" name="memGender" value="femail">
                        ...
                        <div class="login-one-inputs mt-1 check">
                            ...
                            <label class="cbx" for="cbx"><span>
                                ...
                                </svg></span><span class="font-13"><a class="text-primary" href="/member/PrivacyPolicy" target="_blank">개인정보수집및이용약관에 동의합니다</a></span></label>
                        </div>
                    ...
                    <div class="slider-half">
                        <div class="slide-content">
                            <div class="top-sign-up ">
                                <div class="about-comp text-white mt-1">XatoWeb</div>
	...
	var companyCheckBtn = $('#companyCheckBtn');
	var emailFlag = false;
	...
	memId.on("keyup",function(){
		...
		$.ajax({
			...
			success : function(result){
				...
				console.log("중복체크result :" + result);
				if(result === "SUCCESS"){
					$('#memCheck').css('color','red');
					$('#memCheck').text("이미 사용중인아이디입니다!");
					idFlag=false;
				}else{
					console.log("적혀있는값 :"+$('#memId').val());
					const result2 = idTest.test($('#memId').val());  
					console.log("정규식 통과여부 :"+result2);
					if(result2==false){
						$('#memCheck').css('color','red');
						$('#memCheck').text("아이디는 영어로시작하는 영문자 6~20자여야합니다"); 						
						idFlag=false;
					}else{
	...
	companyCheckBtn.on("click",function(){
		var data = {
			    "b_no": [$('#companyNum').val()] // 사업자번호 "xxxxxxx" 로 조회 시,
			   }; 
			   
			$.ajax({
			  url: "https://api.odcloud.kr/api/nts-businessman/v1/status?serviceKey=QzGt9hwjrpFKuNrIKYonMlIxfcN7l9z2DHSTtspqJJNxjBWg64jyFAjPZs0of5%2B6eUiktw%2BEIWTt462biV08BQ%3D%3D",  // serviceKey 값을 xxxxxx에 입력
			  type: "POST",
			  ...
			  success: function(result) {
			      $('#companyStatus').text(result.data[0]["tax_type"]);
			      if(result.data[0]["b_stt"].length>2){
			      companyFlag=true;
			      $('#companyNum').attr('readonly','readonly');
